...
${PROJECT_NAME}`` on Windows or ``propelbldwrap system_project_bd.tcl
${PROJECT_NAME}`` on Linux.
After running the Propel Builder targets we call ``pnmainc system_project.tcl
${PROJECT_NAME}`` on Windows or ``radiantc system_project.tcl ${PROJECT_NAME}``
on Linux. The **PROJECT_NAME** is defined in **Makefile** and used in both
**system_project_bd.tcl** and **system_project.tcl** files.
The **system_project_bd.tcl** runs first. This file is used to create the
**block design project** (Propel Builder) and source the **system_bd.tcl**
which is used for linking one or more corelated block design '.tcl' scripts.
The **system_bd.tcl** is sourced in **adi_project_bd** procedure.
...
.. list-table:: Tools
   ...
   * - Building the project
     - Source the system_project_bd.tcl file in Propel Builder tclsh, source the
